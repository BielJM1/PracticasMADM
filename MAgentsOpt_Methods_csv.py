...
if (__TestNumber__ == 1): # FULL TEST
    ...
    __MDtypes__ = [MDtypes.MD_type1, MDtypes.MD_type2]
    ...
    __AOparams_PI__ = [[[None]], [[None]], [[None]], [[1.00, 0.00], [0.75, 0.15], [0.50, 0.25], [1.00/3, 1.00/3]]] # In case __PhysicalInterference__ is True
...
class Agent:
    ...
    def getBestTaskToMove(self, agents, tasks, curTime, IN, INparamset, PI, PItype, PIparamset, changeTWT, MDtype, UFtype, AOtypeset, AOparamset, realexpMode):
        bestTask  = None
        sbestTask = -1
        tbestTask =  0
        ...
        if ((not self.isTravelling(curTime)) or (self.isTravelling(curTime) and changeTWT)):
            ...
            for taskj in shuffledltasks:
                if (not taskj.isCompleted()):
                    sTask, tTask = self.getTaskStimulus(posi, taskj, agents.getNumberOfAgentsInTask(taskj.id), curTime, IN, INparamset, PI, PItype, PIparamset, MDtype, UFtype, AOtypeset, AOparamset)
    ...
    def getTaskStimulus(self, posi, taskj, taskjcurA, curTime, IN, INparamset, PI, PItype, PIparamset, MDtype, UFtype, AOtypeset, AOparamset):
        ...
        if (MDtype is MDtypes.MD_type1):
            st, ut = self.StUtFunction(taskj, tij, curTime, UFtype) # Here, it is assumed that the agent is located at taski
            if (PI):
                st = self.doAggregation(AOtypeset[0], AOparamset, st, self.getStPI(PItype, taskjcurA, taskj.maxA, PIparamset[0], PIparamset[1]))

        else:
            st, ut = self.StUtFunction(taskj, 0, curTime, UFtype) # Here, it is assumed that the agent is located at taskj
            if (not PI):
...
class Agents:
    ...
    # METHODS
    # Constructor   
    def __init__(self, numAgents, envSize, minAgentVel, minAgentWrkCap, maxAgentWrkCap, iniAgentPosX, iniAgentPosY, tasks, IN, INparamset, PI, PItype, PIparamset, changeTWT, MDtype, UFtype, AOtypeset, AOparamset, realexpMode):
        self.IN          = IN
    ...
    def doAgentsWork(self, tasks, curTime):
        for agent in self.lagents:
            agent.doTaskWork(tasks, curTime, self.UFtype)
    ...
    def moveAgents(self, tasks, curTime):
        ...
        for agent in shuffledlagents:
            bestTask = agent.getBestTaskToMove(self, tasks, curTime, self.IN, self.INparamset, self.PI, self.PItype, self.PIparamset, self.changeTWT, self.MDtype, self.UFtype, self.AOtypeset, self.AOparamset, self.realexpMode)
...
class Tasks:
    ...
    def finishTasks(self, lagents, curTime, UFtype):

        for task in self.ltasks:
            if (not task.isCompleted()):
                task.rET = 0
                lagents[0].calcAchievedTaskUtility(task, curTime, UFtype)
                task.ITC = curTime
...
class optMethod:
    ...
    # METHODS
    # Constructor
    def __init__(self, realexpMode, simendRegFarthestDL, MDtype, IN, INparamset, PI, PItype, PIparamset, changeTWT, randomSeedValue, UFtype, AOtypeset, AOparamset, verboseMode, envXSize, envYSize, numTasks, minDstBtwTasks, minTaskU, maxTaskU, minTaskET, maxTaskET, minTaskDL, maxTaskDL, maxTaskA, numAgents, minAgentVel, minAgentWrkCap, maxAgentWrkCap, iniAgentPosX, iniAgentPosY):
        ...
        self.agents = Agents(numAgents, self.envSize, minAgentVel, minAgentWrkCap, maxAgentWrkCap, iniAgentPosX, iniAgentPosY, self.tasks, self.IN, self.INparamset, self.PI, self.PItype, self.PIparamset, self.changeTWT, self.MDtype, self.UFtype, self.AOtypeset, self.AOparamset, realexpMode)
    ...
    def execute(self):
        ...
        self.tasks.finishTasks(self.agents.lagents, self.clock.getCurrentTime(), self.UFtype)
...
def main(printToFile, verboseMode, realexpMode, simendRegFarthestDL, envXSize, envYSize, numTasks, minDstBtwTasks, minTaskU, maxTaskU, minTaskET, maxTaskET, minTaskDL, maxTaskDL, maxTaskA, numAgents, minAgentVel, minAgentWrkCap, maxAgentWrkCap, iniAgentPosX = None, iniAgentPosY = None):
    ...
    if (printToFile):
        ...
        row      = ["M", "IN", "PI", "At", "Ap", "INp", "PIt", "PIp", "C", "V", "S", "ForcedEND", "T", "U", "TbDL", "D"]
                                    ...
                                    for changeTWT in __ChangeTaskWhileTravelling__:
                                        for UFtype in __UFtypes__:
                                            ...
                                            for randomSeedValue in randomSeedRange:
                                                ...
                                                if(printToFile):
                                                    row = [MDtype, IN, PI, AOtypeset, AOparamset, INparamset, PItype, PIparamset, changeTWT, UFtype, randomSeedValue, cForcedEnd, T, U, TbDL, D]
